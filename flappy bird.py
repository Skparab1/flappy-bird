...
while choice != 'q':
    ...
    # move the bird until the user starts
    try:
        while True:
    ...
    while True:
        try:
            ...
            # store lines above and below for displaying image of crash at end
            ...
            # if user has crashed:
            try: # try is there because sometimes it cant read the line fully because the textx is too showt so i dont want it to crash
                if linecontent[2] == '|' :
                    print('\n'+('_'*100))
            ...
            # make height lower slightly
            height -= 0.5

            # print lower line and score adn player name
            print(lines)
            ...
            # make all lines move towards user by a bit
            ...
            # add bird if bird should be in that line
            bird = '(0)>'
            ...
            # move lower and upper lines foreward and add '-' or ' ' to ends
            lines = lines[1:]
            lines = (lines + ' ') if lines[-1] == '-' else (lines + '-')
            
            # if user hits ceiling or ground, break
            if height <= 0:
        ...
        except: # if user presses control c, move bird up a bit
            height += 2
            time.sleep(0.2)

    # appended = false means that the program has not appended name and score into text file yet
    # this makes it so that if the user keeps pressing control c then it wont write to the text file multiple times
    appended = False
    while True:
        try:

            # prints loss message and image of crash
            print('you have lost')
            print('your score was ', score)
            print(' image of crash below'),print(linecontentabove[:100]),print(linecontentshow[:100]),print(linecontentbelow[:100])
            
            # defines what to add to the textfile
            toadd = name + (' '*(20-len(name))) + str(score) + '\n'

            # adds to text file else creates text file and then adds to it
            try:
            ...
            # writes to text file if it has not yet
            if appended == False:
                try:
                ...
                try:
                    ...
                    blank = ''
                appended = True #makes appended variable true so that it wont write to the text file again
            
            # prints scoreboard and highscore
            print('\n\n Scoreboard: ')
            ...
            # special override commands can clear scoreboard or nto save the current entry
